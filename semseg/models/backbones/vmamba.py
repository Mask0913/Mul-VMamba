...
class MambaCross(nn.Module):
    ...
    def __init__(
        ...
        self.in_normal = nn.LayerNorm(hidden_dim)
        self.ou_normal = nn.LayerNorm(hidden_dim)


    def forward(self, x):
        x = x + self.ou_normal(self.op(self.in_normal(x)))
        return x

class dowmmamba_SS2D_SSM(nn.Module):
    '''
    ...
    def forward(self, x_rgb: torch.Tensor):
        ...
        if self.d_conv > 1:
            x_rgb_conv = self.act(x_rgb)  # (b, d, h, w)
            y_rgb = self.CMA_ssm(x_rgb_conv)
            y_rgb = self.out_proj(y_rgb)
        return y_rgb
...
class down_SSM(nn.Module):
    def __init__(
        ...
        # x proj; dt proj ============================
        self.x_proj_1 = nn.Linear(d_inner, (dt_rank + d_state * 2), bias=False)
        self.dt_proj_1 = self.dt_init(self.dt_rank, self.d_inner, dt_scale, dt_init, dt_min, dt_max, dt_init_floor,
                                      **factory_kwargs)


        # A, D =======================================
        self.A_log_1 = self.A_log_init(self.d_state, self.d_inner, merge=True)  # (D, N)
    ...
    def forward(self, x_rgb: torch.Tensor):
        ...
        x_rgb = x_rgb.permute(0, 2, 1).contiguous()
        ...
        B_rgb = rearrange(B_rgb, "b dstate l -> b 1 dstate l")
        C_rgb = rearrange(C_rgb, "b dstate l -> b 1 dstate l")
